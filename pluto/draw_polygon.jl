...
# ╔═╡ 70bf7e15-4ff7-4cc0-89d4-33273446c4ab
@bind filepicker FilePicker()

# ╔═╡ 63b5403f-f57c-46ee-bda7-bc9294efb4b5
md"# Notebook Utility Functions"

# ╔═╡ 824d9230-5a48-4971-9e2b-439bd2392bd3
macro ifdata(ex)
	esc(quote
		if !isnothing(filepicker)
			$ex
		else
...
scratchdir = @get_scratch!("InteractiveGeo")

# ╔═╡ dc7484b2-7f87-44a6-b8da-7f8e56e92d22
title(depth, name) = h.div(
	Markdown.parse("$('#'^depth) $name");
	style="border-radius: 5px; padding: 1px 12px 1px 12px; background: #155e75; color: #67e8f9;"
)

# ╔═╡ 4066e54a-64c5-472f-8831-816b32b65fef
title(1, "Load Raster Data")

# ╔═╡ cd1225cd-795a-4855-b451-fab339079e1b
# ╠═╡ show_logs = false
@ifdata begin
	file = joinpath(scratchdir, filepicker["name"])
	open(io -> write(io, filepicker["data"]), touch(file), "w");
	r = Raster(file)
...
@ifdata begin 	
	...
	# Resize
	resize_ui = @bind resize confirm(
		Slider(0.1:0.1:1; default=0.1, show_value=true)
	)
...
function haversine(a::LLA, b::LLA; R=6372.8)
    Δlat = b.lat - a.lat
    Δlon = b.lon - a.lon
...
function latlonratio(latlon::LLA)
    lat = latlon.lat
    lon = latlon.lon
    Δlat = haversine(LLA(0,0), LLA(1,0))
    Δlon = haversine(LLA(lat, lon - .5), LLA(lat, lon + .5))
    Δlat / Δlon
end
...
@ifdata begin
	...
	remap_function = remappings[remap]
	z = imresize(remap_function(rsel); ratio=resize)
	x = range(ext_sel.X..., size(rsel, 2))
	y = range(ext_sel.Y..., size(rsel, 1))
	origin = LLA(middle(y), middle(x))
	...
	# p.config.modeBarButtonsToAdd = [
	# 	"drawline", "drawclosedpath", "drawcircle", "drawrect", "eraseshape"
	# ]
